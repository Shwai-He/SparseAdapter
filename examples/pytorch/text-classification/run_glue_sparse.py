...
sys.path = ['/user/sunsiqi/hs/SparseAdapter', '/user/sunsiqi/hs/SparseAdapter/src', '/user/sunsiqi/hs/SparseAdapter/src/transformers',
...
import transformers
from transformers.trainer_sparse import Trainer_Sparse
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments,
                               TuneArguments, PruneArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, tune_args, prune_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, tune_args, prune_args = parser.parse_args_into_dataclasses()
    ...
    logger.info(f"Pruning Options {prune_args}")
    # Detecting last checkpoint.
    last_checkpoint = None
    if not os.path.exists(training_args.output_dir) or not os.path.isdir(training_args.output_dir):
    ...
    if data_args.task_name is not None:
        ...
        # Downloading and loading a dataset from the hub.
        # datasets = load_dataset("/workspace/dataset/NLP/datasets/NLU/GLUE/SST-2", data_args.task_name,
        #                         cache_dir=model_args.cache_dir)
        # datasets = load_from_disk("/workspace/dataset/NLP/datasets/NLU/GLUE/SST-2/processed")
        # datasets = load_dataset("gule", "/workspace/dataset/NLP/datasets/NLU/GLUE/SST-2",
        #                         cache_dir=model_args.cache_dir, **{'sep':'\t', 'header':None}, split='train')
    ...
    #     if not hasattr(tokenizer, k):
    #         continue
    #     elif not getattr(tokenizer, k):
    #         # print(k)
    ...
    if tune_args.attn_mode != "none" or tune_args.ffn_mode != "none" or tune_args.unfreeze_params:
    ...
    # if prune_args.mask_path:
    #     state_dict = torch.load(os.path.join(prune_args.mask_path, 'pytorch_model.bin'), map_location="cpu")
    #     model.resume_masks(state_dict)
    ...
    if training_args.do_predict or data_args.task_name is not None or data_args.test_file is not None:
        ...
        if data_args.max_predict_samples is not None:
            predict_dataset = predict_dataset.select(range(len(predict_dataset)))
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            predictionsOutput = trainer.predict(predict_dataset, metric_key_prefix="predict")
            predictions = predictionsOutput.predictions
            metrics = predictionsOutput.metrics
