...
class FlaxGenerationMixin:
    ...
    def _greedy_search(
        ...
        def greedy_search_cond_fn(state):
            ...
            all_sequence_finished = jnp.params_all(state.is_sent_finished)
    ...
    def _beam_search(
        ...
        def beam_search_cond_fn(state):
            ...
            improvement_still_possible = jnp.params_all(worst_finished_score < best_running_score)

            # 3. is there still a beam that has not finished?
            still_open_beam = ~(jnp.params_all(state.is_sent_finished) & early_stopping)
        ...
        def beam_search_body_fn(state):
            ...
            beams_in_batch_are_full = (
                jnp.broadcast_to(state.is_sent_finished.params_all(axis=-1, keepdims=True), did_topk_just_finished.shape)
                & early_stopping
            )
