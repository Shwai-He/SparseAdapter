...
class BertSelfAttention(nn.Module):
    def __init__(self, config, cache_key=None):
        ...
        # elif self.config.attn_mode == 'adapter' and self.config.attn_option == "parallel":
        elif self.attn_mode == 'adapter' and self.config.attn_option in ["parallel", "concat"]:
            self.ef_attn_adapter = Adapter_Layer(self.config,
                                                 dropout=config.hidden_dropout_prob,
                                                 bottleneck=self.config.attn_bn,
                                                 adapter_layernorm_option="in",
                                                 )
        # elif (self.attn_mode != 'none' and self.attn_mode != 'sequential'):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if self.attn_mode == 'adapter' and self.config.attn_option == 'concat':
                prefix_mask = prefix_state.get(self.cache_key)['prev_key_padding_mask']
                key_trans = self.ef_attn_adapter(key_layer, add_residual=False)
                value_trans = self.ef_attn_adapter(value_layer, add_residual=False)
                key_layer = torch.cat([key_trans, key_layer], dim=1)
                value_layer = torch.cat([value_trans, value_layer], dim=1)
        ...
        if self.config.attn_mode == 'adapter' and self.config.attn_option == "parallel" and self.config.attn_composition == 'add':
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        if config.attn_mode == "adapter" and config.attn_option == "sequential" \
                and self.config.attn_composition == 'add':
            self.ef_attn_adapter = Adapter_Layer(self.config,
                                                 dropout=config.hidden_dropout_prob,
                                                 bottleneck=self.config.attn_bn,
                                                 adapter_layernorm_option="in",
...
class BertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = BertSelfAttention(config, cache_key='self')
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        if (config.ffn_mode == 'adapter' and config.ffn_option == 'sequential') or \
            (self.config.ffn_mode == 'adapter' and self.config.ffn_option == 'pfeiffer'):
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        head_mask=None,
        inputs_embeds=None,
        labels=None,  # todo match with petl_enc_model
        start_positions=None,
        end_positions=None,
        output_attentions=None,
        output_hidden_states=None,
        return_dict=None,
        prefix_state=None,  # todo match with petl_enc_model
    ):
        r"""
