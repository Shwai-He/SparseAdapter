...
class Trainer_Sparse:
    ...
    def get_prune_dataloader(self) -> DataLoader:
        ...
        batch_size = self.args.train_batch_size if self.prune_args.pruner != 'grasp' \
                                    else self.args.train_batch_size // 2
    ...
    def train(
        ...
        **kwargs,
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            # todo pruing the weight in adapter
            prune_epoch = epoch - self.prune_args.begin
            if (self.prune_args.sparsity < 1.) and (sparse > self.prune_args.sparsity) and (prune_epoch >= 0):
                pruner = self.pruner(self.prune_args.pruner)(masked_parameters(model))
                pruner.optimizer = self.optimizer
                sparse = self.prune_args.sparsity ** ((prune_epoch + 1) / int(self.prune_args.prune_iterations))
                prune_loop(model, pruner, prune_dadaloader, model.module.device, sparse,
                           self.prune_args.compression_schedule, self.prune_args.mask_scope,
                           self.prune_args.prune_epochs, self.prune_args.structured,
                ...
                logger.info(
                    f"Sparsity is updated, now it is {sparse}."
                    )
